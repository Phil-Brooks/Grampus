...
module Move =
    ...
    let ``Move.Create correctly encodes and decodes basic move data`` (f, t, p, c) =
        ...
        Move.From mv |> should equal f
        Move.To mv |> should equal t
        Move.MovingPiece mv |> should equal p
        Move.CapturedPiece mv |> should equal c
    ...
    let ``Move.CreateProm correctly encodes promotion data`` () =
        ...
        Move.PromoteType mv |> should equal prom
    ...
    let ``MovingPlayer matches the color of the moving piece`` (p: Piece) =
