...
module Bitboard =
    let DebrujinPositions =
        [| 0s; 1s; 28s; 2s; 29s; 14s; 24s; 3s; 30s; 22s; 20s; 15s; 25s; 17s; 4s; 
           8s; 31s; 27s; 13s; 23s; 21s; 19s; 16s; 7s; 26s; 12s; 18s; 6s; 11s; 5s; 
           10s; 9s |]
    ...
    let ShiftDirN(ibb : Bitboard) =
        (uint64 (ibb &&& ~~~Bitboard.Rank8) <<< 8) |> BitB
    let ShiftDirE(ibb : Bitboard) =
        (uint64 (ibb &&& ~~~Bitboard.FileH) >>> 1) |> BitB
    let ShiftDirS(ibb : Bitboard) =
        (uint64 (ibb &&& ~~~Bitboard.Rank1) >>> 8) |> BitB
    let ShiftDirW(ibb : Bitboard) =
        (uint64 (ibb &&& ~~~Bitboard.FileA) <<< 1) |> BitB
    let ShiftDirNE(ibb : Bitboard) =
        (uint64 (ibb &&& ~~~Bitboard.Rank8 &&& ~~~Bitboard.FileH) <<< 9) |> BitB
    let ShiftDirSE(ibb : Bitboard) =
        (uint64 (ibb &&& ~~~Bitboard.Rank1 &&& ~~~Bitboard.FileH) >>> 7) |> BitB
    let ShiftDirSW(ibb : Bitboard) =
        (uint64 (ibb &&& ~~~Bitboard.Rank1 &&& ~~~Bitboard.FileA) >>> 9) |> BitB
    let ShiftDirNW(ibb : Bitboard) =
        (uint64 (ibb &&& ~~~Bitboard.Rank8 &&& ~~~Bitboard.FileA) <<< 7) |> BitB
    ...
    let ContainsPos (pos : Square) ibb =
        (ibb &&& (pos |> Square.ToBitboard)) <> Bitboard.Empty
    ...
    let GetFirstPos ibb : Square =
        ...
        if num <> 0UL then 
            let number = int (num)
            DebrujinPositions.[int 
                                   ((uint32 (number &&& -number) * 0x077CB531u) 
                                    >>> 27)]
        else 
